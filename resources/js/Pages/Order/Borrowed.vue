...
<template>
    <Head :title="isEn ? 'Borrowed Books' : 'كتب مستعارة'"/>
        ...
        <div class="container">
            <div class="my-shadow rounded-xl grid center-container w-full gap-5 p-5" >
                ...
                <div class="grid grid-cols-2">
                    ...
                    <Link :href="route('orders.reverse',[order.id])" method="patch" class="justify-self-center" preserve-scroll>
                        <PrimaryButton class="">{{  $page.props.isEn ? "Reverse" : "ارجاع"  }}</PrimaryButton>
                    </Link>
                </div>
            </div>
        </div>
    ...
    <div class="" v-if="orders == false">
        <div class="container">
            <div class="my-shadow rounded-xl grid center-container w-full gap-5 p-5" >
                <h3 class="text-center">{{ isEn ? "No Borrowed Books" : "لا توجد كتب مستعارة" }}</h3>
            </div>
        </div>
    </div>
</template>
